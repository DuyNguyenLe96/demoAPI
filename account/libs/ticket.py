...
from ..utils.strs import get_random_string
...
def take_user_id_from_ticket(ticket):
    ...
    userID = base64.b64decode(ticket_has_split[1] + '=' * (-len(ticket_has_split[1]) % 4))
    return userID.decode('ascii')


def take_life_time_from_ticket(ticket):
    ticket_has_split = ticket.split('.')
    lifetime = base64.b64decode(ticket_has_split[2] + '=' * (-len(ticket_has_split[2]) % 4))
    return lifetime.decode('ascii')


def take_device_id_from_ticket(ticket):
    ticket_has_split = ticket.split('.')
    device_id = base64.b64decode(ticket_has_split[3] + '=' * (-len(ticket_has_split[3]) % 4))
    return device_id.decode('ascii')


def ticket_is_valid(ticket, session_ticket):
    ...
    if ticket != session_ticket or diff_time > settings.TICKET_LIFETIME:
        return False
    return True
