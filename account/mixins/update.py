...
class UpdateMixin(serializers.ModelSerializer):
    def get_extra_kwargs(self):
        ...
        allow_update_fields = getattr(self.Meta, 'allow_update_fields', None)
        ...
        if self.instance and allow_update_fields:
            field_names = self._get_field_names()
            for field in field_names:
                if field not in allow_update_fields:
                    kwargs.setdefault(field, {})
    ...
    def _get_field_names(self):
        ...
        if exclude:
            field_names = set(self._get_all_field_names()) - set(exclude)
        return field_names
    ...
    def _get_all_field_names(self):
        ...
        field_names = (
                [info.pk.name] + list(info.fields) + list(info.forward_relations)
        )
        return field_names
